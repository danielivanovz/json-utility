...
export const App = () => {
	...
	const [isJSON, setError] = useState<boolean>(false);
	...
	const updateQuery = () => {
		...
		if( isJSON ) {
			console.log(isJSON)
			JSON.stringify(JSON.stringify(JSON.parse(query!)))
		}
	...
	const handlePaste = (e: ClipboardEvent): any => {
		const paste = e.clipboardData!.getData('Text').replace(/\s*$/,'');
		isJSONCheck(paste) ? setQuery(paste) : setError(false)
	}
		...
		<div className='flex flex-col p-14 h-screen items-center justify-center bg-gray-50 '>
			...
			<div className='w-full h-full flex m-2 p-10 rounded-md drop-shadow-2xl'>
				...
				<div className='h-full w-full mx-5 text-sm text-black border border-gray-200 shadow-xl rounded-sm'>
					...
					<CopyToClipboard 
					//@ts-ignore
					text={JSON.stringify(JSON.stringify(JSON.parse(json)))} onCopy={()=> {setState(true)}}>
